...
import { FirebaseMessaging, sendAPN } from '../controller/sentNotification';
import { Permission, _permissions } from './permission';
...
export function BaseResolver<T, P>(
  ...
  abstract class BaseResolver {
    ...
    protected fitlerByUserId(filter: any, user: any) {
      ...
      if (
      ...
      } else if (user.type === 'Viewer' && objType.name === 'Delivery') {
        newFilter = { 'courier._id': userId };
      } else if (user.type === 'User' && objType.name === 'Delivery') {
        newFilter = { 'merchant._id': userId };
      } else if (user.type === 'User' && objType.name === 'Product') {
        newFilter = {
          $or: [
            { 'seller._id': userId.toString() },
        ...
        newFilter = { _createdby: user.username };
      }
      if (!!newFilter) {
        if (keys.length === 0) {
          filter = newFilter;
    ...
    async getOne(
      ...
      } else throw new Error("Don't have permission to read an item");
    }
    ...
    protected permissionCheck(
      ...
      let index: number = _permissions.findIndex(
        (p) => p.listname === listname
      );
    ...
    async deleteItem(
      ...
      if (
        ...
        //}
        return deleted;
      } else throw new Error("Don't have permission to delete item");
    }
    ...
    protected async newItem(
      ...
      if (
        ...
        return saved;
      } else throw new Error("Don't have permission to create an item");
    }
